...
const ProductPageContainer: FC<Props> = ({ productData, sizeId, children }) => {
    ...
    const { productId, variantId, weight, variantSlug, shippingDetails, images, sizes } = productData;
    // Failed to get shippingdetails (defined in getShippingDetails, fallback to false), return null
    ...
    const handleAddToCart = () => {
        if (maxQty <= 0) return;
        if (!isProductValid) {
            toast.error("Please choose a valid size and quantity!");
            return;
        }
                        ...
                        <div className="z-20">
                            <div className="bg-white border rounded-md overflow-hidden overflow-y-auto p-4 pb-0">
                                ...
                                <div className="mt-5 bg-white bottom-0 pb-4 space-y-3 sticky">
                                    ...
                                    {/* Action buttons */}
                                    <button
                                        className={cn("relative w-full py-2.5 min-w-20 bg-[var(--orange-background)] hover:bg-[var(--orange-hover)] text-white h-11 rounded-3xl leading-6 inline-block font-bold whitespace-nowrap border border-orange-border cursor-pointer transition-all duration-300 ease-bezier-1 select-none",
                                            {
                                    ...
                                    <button
                                        className={cn(
                                            "relative w-full py-2.5 min-w-20 bg-[var(--orange-border)] hover:bg-[#e4cdce] text-[var(--orange-hover)] h-11 rounded-3xl leading-6 inline-block font-bold whitespace-nowrap border border-orange-border cursor-pointer transition-all duration-300 ease-bezier-1 select-none",
                                            {
                                                "cursor-not-allowed": !isProductValid || maxQty <= 0,
                                            }
                                        )}
                                    ...
                                    <button className="relative w-full py-2.5 min-w-20 bg-[var(--orange-background)] hover:bg-[var(--orange-hover)] text-white h-11 rounded-3xl leading-6 inline-block font-bold whitespace-nowrap border border-orange-border cursor-pointer transition-all duration-300 ease-bezier-1 select-none"
