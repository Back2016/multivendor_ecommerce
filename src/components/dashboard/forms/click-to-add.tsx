...
const ClickToAddInputs = <T extends Detail>({
    ...
    const handleRemove = (index: number) => {
        ...
        // When remove a detail, for color, auto reset colorPickerIndex to null so when add it back it is not open
        // Reset colorPickerIndex if the open one is being removed
        if (colorPickerIndex === index) {
            setColorPickerIndex(null);
        }
        // Shift index down if a prior row is removed
        if (colorPickerIndex !== null && colorPickerIndex > index) {
            setColorPickerIndex((prev) => (prev !== null ? prev - 1 : null));
        }
    ...
    const MinusButton = ({ onClick, disabled }: { onClick: () => void, disabled: boolean }) => {
        let btnClassName, svgClassName;
        if (disabled) {
            btnClassName = "group outline-none";
            svgClassName = "w-8 h-8 stroke-gray-400 fill-none";
        } else {
            btnClassName = "group cursor-pointer outline-none hover:rotate-90 duration-300";
            svgClassName = "w-8 h-8 stroke-blue-400 fill-none group-hover:fill-white group-active:stroke-blue-200 group-active:fill-blue-700 group-active:duration-0 duration-300";
        }
        return (
            <button
                type="button"
        ...
        <div className="flex flex-col gap-y-4">
            ...
            {details.map((detail, index) => (
                <div key={index} className="flex items-center gap-x-4">
                    ...
                    {/* If only 1 detail, not allow to remove */}
                    <MinusButton onClick={() => handleRemove(index)} disabled={details.length <= 1} />
