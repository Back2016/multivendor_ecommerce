        ...
        return (
            <div className="max-w-8xl">
                <div className={cn("grid h-[800px] overflow-hidden bg-white rounded-md", GridClassName)}>
                    {
                        images.map((img, i) => (
                            <div key={i} className={cn("relative group h-full w-full border border-gray-300",
                                `grid_${imagesLength}_image_${i + 1}`, {
                                "h-[266.67px]": images.length === 6
                            }
                            )}>
                                {/* Image */}
                                <Image
                                ...
                                >
                                    ...
                                    <ColorPalette
                                        colors={colors}
                                        setColors={setColors}
                                        // only send the colors for the current image, [["red"], ["yellow", "blue"]] <-- [][]
